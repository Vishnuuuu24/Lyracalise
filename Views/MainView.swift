...
struct MainView: View {
    ...
    var body: some View {
                    ...
                    ScrollViewReader { proxy in
                        ScrollView {
                            VStack(alignment: .center, spacing: 18) {
                                // Buffer views to allow scrolling active line to center
                                ...
                                ForEach(Array(lyricSyncManager.loadedLyrics.enumerated()), id: \.offset) { index, entry in
                                    Text(entry.line)
                                        ...
                                        .id(index) // Assign an ID for ScrollViewProxy
                                        .onTapGesture {
                        ...
                        .onChange(of: lyricSyncManager.currentLineIndex) { newIndex in
                            // When the current line changes, scroll to it.
        ...
        .onAppear {
            // The LyricSyncManager initializer now handles starting observation,
            // but we need to call the version with the timeout.

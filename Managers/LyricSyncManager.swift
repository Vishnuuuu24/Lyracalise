...
class LyricSyncManager: ObservableObject {
    ...
    // Remove Resume Auto-Sync logic/button
    // Add a robust Re-Sync function
    func reSync(with track: SpotifyTrack?) {
        // Always force a full reload and resume normal operation
        if let track = track {
            ...
            self.lastProgressMs = track.progressMs ?? 0
            self.syncState = (track.isPlaying ?? true) ? .playing : .paused
            self.loadLyricsForCurrentSong()
            self.setLyricSyncOffset(milliseconds: track.progressMs ?? 0)
            self.statusMessage = ""
        } else {
            self.syncState = .waiting
            self.pauseLyricTimer(at: 0)
            self.statusMessage = "Waiting for Spotify..."
        }
        // Spotify polling/observation continues as normal
    }
    ...
    func startLyricSync() {
        ...
        // Fetch the latest Spotify track info and start syncing
        SpotifyManager.shared.fetchCurrentTrack { [weak self] track in
            ...
            DispatchQueue.main.async {
                ...
                // Resume Spotify polling/observation
                self.startSpotifyObservation()
            }
        }
    }
    ...
    func stopAll(showStopped: Bool = false) {
        // End live activity
        stopLiveActivity()
    ...
    // Stop button: stop everything except Spotify polling/observation
    func stopAllLyricSync() {
        ...
        self.statusMessage = "Stopped."
        self.stopSyncing() // Stop the lyric sync timer as well
        // Do NOT stop Spotify polling/observation here (now handled above)
    }
        ...
        if SpotifyManager.shared.isLoggedIn {
            spotifyTimer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { [weak self] _ in
                SpotifyManager.shared.fetchCurrentTrack { track in
                    guard let self = self else { return }
                    if self.isStopped { return } // Do not update lyrics if stopped
                    if let track = track {
                        DispatchQueue.main.async {
                        ...
                        // No track data, show waiting state
                        self.syncState = .waiting
                        self.pauseLyricTimer(at: 0)
                        self.statusMessage = "Waiting for Spotify..."
                    }
    ...
    private func handleSpotifyState(track: SpotifyTrack) {
        ...
        let fallbackTitleMatch = self.nowPlayingTitle.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == track.name.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        let fallbackArtistMatch = self.nowPlayingArtist.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == track.artist.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        let shouldReset = (track.id != "" && trackIDChanged) || (track.id == "" && (!fallbackTitleMatch || !fallbackArtistMatch))
        let isPlaying = track.isPlaying ?? true
        let currentProgress = track.progressMs ?? 0
        // Always force a full reset if track changes
        if shouldReset {
            self.resetStateForNewSong()
            self.nowPlayingTitle = track.name
            self.nowPlayingArtist = track.artist
            ...
            self.statusMessage = ""
            // Always start a new live activity for a new song
            self.startLiveActivity()
            return
        }
        // Seek detection (only if jump > 2s)
        ...
        case (.paused, true), (.seeking, true), (.waiting, true), (.stopped, true):
            ...
            // Only resync on play if coming from paused/seeking/waiting/stopped and there was a significant jump
            if let last = self.lastProgressMs, abs(currentProgress - last) > 2000 {
                self.setLyricSyncOffset(milliseconds: currentProgress)
            } else {
                // Just resume the lyric timer from where it left off
                self.manualSyncStartTime = Date()
                self.startLyricTimerIfNeeded()
            }
        default:
            break
        }
        ...
        if currentProgress == 0 && !isPlaying {
            self.syncState = .stopped
            self.pauseLyricTimer(at: 0)
            self.stopLiveActivity() // End live activity when song ends
        }
    }
    // Helper to pause lyric timer
    private func pauseLyricTimer(at progressMs: Int) {
        self.spotifyTimer?.invalidate()
        self.spotifyTimer = nil
        // Store the offset so resume works
        self.manualSyncOffset = Double(progressMs) / 1000.0
        self.manualSyncStartTime = nil
    }
    // Helper to resume lyric timer
    private func resumeLyricTimer(at progressMs: Int) {
        // Remove resumeAutoSync and related functions
    }
    // Helper to set lyric sync offset
    private func setLyricSyncOffset(milliseconds: Int) {
        self.manualSyncOffset = Double(milliseconds) / 1000.0
        self.manualSyncStartTime = Date()
    }
    // Dummy function to start lyric timer if needed (replace with your actual logic)
    private func startLyricTimerIfNeeded() {
        // ... existing code to start the lyric timer ...
    }
}
